...
export class Resource<T> {
  ...
  constructor(
    ...
    initial?: T,
    /**
     * Whether this can load, and if not, whether that is temporary.
     * `waiting` reads as pending — something is coming — while `never`
     * settles empty: nothing is coming and nothing went wrong, which is
     * what a reader who has signed out is owed instead of an error.
     */
  ...
  subscribe = (listener: () => void): (() => void) => {
    ...
    // A failure is not a reason to try again on its own: a second element
    // reading the same query, or a card remounting on a tab switch, would
    // otherwise re-issue a request that has already failed once. Retry is
    // the way back, and `refetch` is what it calls.
    if (
      !this._loaded &&
  ...
  load(): Promise<void> {
    ...
    if (readiness !== "ready") {
      // `never` settles: a resource constructed pending, or reset into it,
      // would otherwise show a skeleton for a load that is not coming.
      // `waiting` keeps the skeleton, which is what it is for.
    ...
    // The error goes when the load starts, not when it lands: an element
    // shows its error branch in preference to its pending one, so a retry
    // that kept the old error would look like a button that does nothing.
  ...
  update(fn: (data: T) => T): void {
    ...
    // Retires whatever is on the wire, as seed and reset do: a request that
    // landed afterwards would merge into the data this replaced and drop the
    // update on the floor.
    this._generation++;
    this._inflight = undefined;
    this._extending = undefined;
  ...
  /**
   * Forgets everything and stays empty: no data, no error, nothing pending.
   * What a resource holds when there is no session to hold it for — the
   * reader signed out, and an empty card is the honest answer where an
   * error would be a complaint about their own page.
   */
  clear(): void {
    this._generation++;
    this._inflight = undefined;
  ...
  reset(): void {
    ...
    this._extending = undefined;
    this._loaded = false;
    // Pending while something is about to happen, or is waited on: only a
    // resource with nothing coming settles empty.
    const readiness = this._readiness();
...
export interface KeyedResourceOptions<P> {
  ...
  /**
   * Whether entries can load, and if not, whether that is temporary; every
   * entry, however it was reached, asks before fetching. Defaults to ready.
   */
  readiness?: () => Readiness;
}

/**
 * How many unrendered entries a keyed resource keeps.
 *
...
export class KeyedResource<T, P> {
  ...
  /** Loads every subscribed entry that has nothing yet. */
  resumeAll(): void {
  ...
  /** Forgets every entry's data and leaves it empty; nothing reloads. */
  clearAll(): void {
    for (const [key, { resource }] of Array.from(this._entries)) {
      if (resource.subscriberCount === 0) {
        this._entries.delete(key);
