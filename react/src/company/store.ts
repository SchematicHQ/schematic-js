...
export class Resource<T> {
  ...
  constructor(
    ...
    initial?: T,
    /**
     * Whether there is anything to load for. A resource that cannot load
     * stays empty rather than reporting the refusal as a failure: the
     * session ended, and its reader is owed a blank card rather than an
     * error about a page they have signed out of.
     */
    private readonly _canLoad: () => boolean = () => true,
  ) {
  ...
  load(): Promise<void> {
    ...
    if (!this._canLoad()) {
      // Settle: a resource constructed pending, or reset into it, would
      // otherwise show a skeleton for a load that is never coming.
      if (this._snapshot.isPending) {
  ...
  reset(): void {
    ...
    this._extending = undefined;
    this._loaded = false;
    // Pending only if something is actually about to happen: a resource
    // that cannot load would otherwise sit showing a skeleton forever.
    const willLoad = this._listeners.size > 0 && this._canLoad();
    this._set({ data: undefined, error: undefined, isPending: willLoad });
    if (willLoad) {
      void this.load();
    }
  }
...
export interface KeyedResourceOptions<P> {
  ...
  /**
   * Whether there is anything to load for; every entry, however it was
   * reached, asks before fetching. Defaults to always.
   */
  canLoad?: () => boolean;
}
...
export class KeyedResource<T, P> {
  ...
  private readonly _canLoad: () => boolean;
  ...
  constructor(
    ...
    this._canLoad = options.canLoad ?? (() => true);
