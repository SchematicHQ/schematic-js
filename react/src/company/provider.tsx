...
interface CompanyProviderDataProps {
  ...
  /**
   * Which company is being read, and the credential for it:
   * `{ key, token }` while someone is signed in, `null` when nobody is, and
   * `undefined` while the host is still finding out.
   *
   * The key is the identity, so a token that rotates — a provider called
   * again, a closure rebuilt on a render — is the same session and keeps
   * what is loaded, while a different key drops it. `null` drops it too and
   * reads nothing until a session returns; `undefined` states nothing, which
   * is what an auth resolving asynchronously renders first.
   */
...
export function CompanyProvider({
  ...
  // A host that writes `token: async () => …` inline hands over a new
  // function on every render, and the behaviour behind each one is the same.
  // So the client is given a single provider that reads the latest prop and
  // never sees the churn. Written during render, not in an effect: the
  // client can call the provider from a fetch a render-phase install has
  // already set going, which is before any effect of this component has run.
  //
  // Only an active session sets it, and only an ended one clears it: a host
  // that says `undefined` states nothing, so the token behind the session in
  // hand has to stay callable — a refresh or a 401 retry landing during that
  // window would otherwise ask a provider that has been emptied.
  const latestToken = useRef<AccessToken | undefined>(undefined);
  if (session === null) {
    latestToken.current = undefined;
  } else if (session !== undefined) {
    latestToken.current = session.token;
  }
  ...
  // The client decides what a statement means and ignores one that changes
  // nothing, so this forwards on every pass rather than keeping its own
  // record of what it last said.
  const install = () => {
    companyClient?.setSession?.(forwarded);
  };
  ...
  // is listening yet and `setSession` can reach no mounted component.
  //
  // Marked whether or not the statement changed anything: a pass that said
  // nothing has still had its chance, and leaving the branch armed would let
  // a session arriving later — after the children have subscribed — reset
  // the store from inside a render.
  ...
  const source = useMemo<CompanyDataSource | undefined>(() => {
    ...
    const handle = <K extends CompanyResourceName>(
      ...
      // Deleted first so the write lands at the end: `set` on a key that is
      // already there keeps its original position, which would leave the
      // entry being read most often sitting at the front as the one eviction
      // takes next.
      handles.delete(cacheKey);
