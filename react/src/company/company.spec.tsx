...
import { CompanyStore, type CompanyClient, type SessionEvent } from "./client";
...
describe("company hooks", () => {
  ...
  it_("does not reset a listening store from inside a render", async () => {
    ...
    const at = (key: string) => (
      <CompanyProvider companyClient={client} session={{ key, token: "t" }}>
        <Reader />
      </CompanyProvider>
    );
  ...
  it_("signs out and back in through a real client", async () => {
    ...
    const at = (
      session: React.ComponentProps<typeof CompanyProvider>["session"],
    ) => (
      <CompanyProvider companyClient={client} session={session}>
        <Reader />
      </CompanyProvider>
    );
    ...
    view.rerender(at({ key: "company_a", token: "t_a" }));
    await flush();
    ...
    view.rerender(at({ key: "company_b", token: "t_b" }));
    await flush();
    expect(view.container.textContent).toBe("ok");
  ...
  it_(
    ...
    async () => {
      ...
      await flush();
      expect(stated()).toContainEqual({ key: "company_a", token: "t_a" });

      view.rerender(
        <CompanyProvider companyClient={client} session={null}>
          <span />
        </CompanyProvider>,
      );
      await flush();
      ...
      view.rerender(
        <CompanyProvider companyClient={client} session={undefined}>
          <span />
        </CompanyProvider>,
      );
      await flush();
    ...
    async () => {
      ...
      const forwarded = () =>
        (client.setSession as ReturnType<typeof vi.fn>).mock.calls
          .map((call) => call[0])
          .filter((value): value is { key: string; token: () => unknown } =>
            Boolean(value),
          );
      ...
      const view = render(
        <CompanyProvider
          companyClient={client}
          session={{ key: "company_a", token: async () => "t1" }}
        >
          <span />
        </CompanyProvider>,
      );
      ...
      view.rerender(
        <CompanyProvider
          companyClient={client}
          session={{ key: "company_a", token: async () => "t2" }}
        >
          <span />
        </CompanyProvider>,
      );
      ...
      await expect(tokens[0]()).resolves.toBe("t2");
    },
  );
