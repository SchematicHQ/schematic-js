...
> & {
  ...
  /**
   * The company session: which company the `/company/*` endpoints are read
   * for and the credential for it, `null` when nobody is signed in, and
   * `undefined` while the host is finding out. Held alongside the
   * publishable key, which keeps serving flags.
   */
...
export const SchematicProvider: React.FC<SchematicProviderProps> = ({
  ...
  const resolvedCompanyClient = useMemo(
    ...
    // The session is forwarded through CompanyProvider's setSession; only
