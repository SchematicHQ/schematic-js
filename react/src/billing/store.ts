...
export class Resource<T> {
  ...
  constructor(
    ...
    initial?: T,
    /** `never` settles empty: nothing is coming and nothing went wrong. */
  ...
  subscribe = (listener: () => void): (() => void) => {
    ...
    // reading the same query, or a card remounting, would re-issue a request
    // that already failed. `refetch` is the way back.
    if (
      !this._loaded &&
  ...
  load(): Promise<void> {
    ...
    // Cleared when the load starts, not when it lands: an element prefers
    // its error branch to its pending one, so a retry that kept the old
    // error would look like a button that does nothing.
  ...
  /** A no-op until the resource has data. */
 ...
 * set is its own row set with its own paging, so it gets its own `Resource`;
 * there is no merging across keys.
 */
