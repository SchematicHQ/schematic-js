...
describe("Resource", () => {
  ...
  it("clears the error when another page is asked for", async () => {
    // The same reason as a retry: a failed "Load more" that keeps its error
    // through the next attempt reads as a button that does nothing.
    let fail = false;
  ...
  it("clears the error when a retry starts, so the retry is visible", async () => {
    // An element shows its error branch in preference to its pending one, so
    // an error left standing through the retry makes the button look dead.
    let fail = true;
    const resource = new Resource(async () => {
      if (fail) throw new Error("boom");
      return "good";
...
describe("a resource whose load failed", () => {
  it("does not try again for every new reader", async () => {
    // Two elements on the same query, or a card remounting on a tab switch:
    // the error is already on screen behind a Retry button, and re-issuing
    // the request per subscriber turns one failure into a burst.
...
describe("a resource that cannot load yet", () => {
  it("waits, holding its pending state for what is coming", async () => {
    ...
    // the card keeps its skeleton rather than reporting an empty history.
    const fetcher = vi.fn(async () => "data");
  ...
  it("resets to a skeleton while waiting, and to empty when never", async () => {
...
describe("KeyedResource", () => {
  ...
  it("clears every entry that is rendered and drops the rest", async () => {
    ...
    // The unrendered one is dropped rather than kept empty.
    expect(k.has({ q: "b" })).toBe(false);
    await flush();
    expect(fetcher).toHaveBeenCalledTimes(1);
  });
  ...
  it("resumes only what is rendered and has nothing", async () => {
    ...
    // An entry nothing renders is left alone.
    k.get({ q: "c" });
    k.resumeAll();
    await flush();
    expect(fetcher).toHaveBeenCalledTimes(2);
