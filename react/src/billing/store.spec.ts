...
describe("Resource", () => {
  ...
  it("clears the error when another page is asked for", async () => {
    // The same reason as a retry: an error left standing through the next
    // attempt makes a fresh page indistinguishable from nothing happening.
    let fail = false;
    const resource = new Resource(async () => "a");
    await resource.load();
    fail = true;
    await resource.extend(async (data) => {
      if (fail) throw new Error("page failed");
      return `${data}b`;
    });
    expect(resource.getSnapshot().error).toEqual(new Error("page failed"));

    fail = false;
    const retry = resource.extend(async (data) => `${data}b`);
  ...
  it("clears the error when a retry starts, so the retry is visible", async () => {
    // A retry has to read as pending from the moment it is asked for, not
    ...
    await resource.load();
    expect(resource.getSnapshot().error).toEqual(new Error("boom"));

    fail = false;
    const retry = resource.refetch();
    expect(resource.getSnapshot()).toMatchObject({
      error: undefined,
      isPending: true,
    });
  ...
  it("extends the data it has and reads as pending while it loads", async () => {
    const resource = new Resource(async () => "a");
    await resource.load();
  ...
  it("keeps the data it has when an extend fails, and never rejects", async () => {
    const resource = new Resource(async () => "a");
    await resource.load();
    await expect(
...
describe("a resource whose load failed", () => {
  it("does not try again for every new subscriber", async () => {
    // Two subscribers on the same query, or one remounting: the error is
    // already recorded, and re-issuing the request per subscriber turns one
    // failure into a burst.
    ...
    await resource.refetch().catch(() => undefined);
    expect(fetcher).toHaveBeenCalledTimes(2);
  });
});

describe("a resource that cannot load yet", () => {
  it("waits, holding its pending state for what is coming", async () => {
    // `waiting` is a session still resolving: something is on its way, so
    // the resource stays pending rather than reporting an empty history.
    const fetcher = vi.fn(async () => "data");
    let readiness: "ready" | "waiting" | "never" = "waiting";
    const resource = new Resource(fetcher, undefined, () => readiness);
    resource.subscribe(() => {});
    await flush();
    expect(fetcher).not.toHaveBeenCalled();
    expect(resource.getSnapshot()).toEqual({
  ...
  it("settles empty when nothing is coming", async () => {
    // `never` is a session that has ended: empty and settled, not pending
    // for a load that will not happen and not an error about signing out.
    const fetcher = vi.fn(async () => "data");
    const resource = new Resource(fetcher, undefined, () => "never");
    resource.subscribe(() => {});
    await flush();
    expect(fetcher).not.toHaveBeenCalled();
    expect(resource.getSnapshot()).toEqual({
  ...
  it("resets to pending while waiting, and to empty when never", async () => {
    let readiness: "ready" | "waiting" | "never" = "ready";
    const resource = new Resource(
      async () => "data",
      undefined,
      () => readiness,
  ...
  it("clears what it holds without reloading", async () => {
    const fetcher = vi.fn(async () => "data");
    const resource = new Resource(fetcher);
    resource.subscribe(() => {});
    await flush();
    ...
    await flush();
    // Cleared, not reset: nothing reloads behind it.
    expect(fetcher).toHaveBeenCalledTimes(1);
  });

  it("drops a load that lands after a clear", async () => {
    ...
    release?.("late");
    await flush();
    // The answer belonged to the state that was cleared.
    expect(resource.getSnapshot().data).toBeUndefined();
  });
});
...
describe("KeyedResource", () => {
  ...
  it("clears every entry that is rendered and drops the rest", async () => {
    const { keyed: k, fetcher } = keyed();
    k.subscribe({ q: "a" }, () => {});
    k.get({ q: "b" });
    await flush();
    ...
    k.clearAll();
    expect(k.get({ q: "a" }).getSnapshot()).toEqual({
      data: undefined,
      error: undefined,
      isPending: false,
    });
  ...
  it("resumes only what is rendered and has nothing", async () => {
    const { keyed: k, fetcher } = keyed();
    k.subscribe({ q: "a" }, () => {});
    await flush();
    expect(fetcher).toHaveBeenCalledTimes(1);

    // Already has data: resuming is not refetching.
    k.resumeAll();
    await flush();
    expect(fetcher).toHaveBeenCalledTimes(1);
