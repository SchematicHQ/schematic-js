...
export class BillingStore {
  ...
  constructor(
    ...
    this.invoices = new KeyedResource(
      ...
      // A session still pending holds the skeleton; one that has ended
      // settles empty rather than rendering the client's refusal as an error.
      { readiness: () => READINESS[this._client.sessionStatus] },
    );
    // A session the host has not resolved yet is no reason to drop a
    // prefetch — `connect` checks it once the session says who it is.
    if (
      initialData.invoices !== undefined &&
      seedBelongs(initialData.sessionKey, this._client)
    ) {
      // Normalized the way the hook normalizes, or the element asking it
      // fetches the same page again.
      this.invoices.seed(
  ...
  connect(): () => void {
    ...
    const unsubscribe = this._client.onSessionChange?.((event) => {
      if (event.type === "ended") {
        ...
        // A session has arrived and the rows on screen name another
        // session — a cached page, or a second tab that switched. They are
        // not this one's to show. A prefetch that named nobody is taken at
        // face value; see `seedBelongs`.
        this._seeded = false;
        this.resetAll();
      } else {
        this._seeded = false;
