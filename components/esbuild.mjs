...
const options = {
  ...
  format,
  outfile,
  // The v3 entry must NOT inline the schematic SDKs: the hooks resolve the
  // consumer's SchematicProvider through a module-level React context, and
  // a bundled copy would read a different context instance than the
  // provider writes — every element would render its missing-provider
  // error. They are declared runtime dependencies instead.
  external:
    entry === "v3"
      ? [
          "react",
  ...
  // The v3 stylesheet inlines the schematic-icons icon-font CSS (see
  // src/v3/styles.tsx); import it as a plain string. The main entry never
  // imports .css, so this loader only affects the v3 bundle.
  loader: { ".css": "text" },
