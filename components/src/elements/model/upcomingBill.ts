...
export interface DiscountLine {
  /** The coupon's name, for a host that wants to show it. */
  couponName: string;
...
export interface UpcomingBillSummary {
   ...
   * negative and its text carries the locale's sign. `null` when none of the
   * balance is applied.
   */
...
export function deriveUpcomingInvoice(
  ...
  const dueDate = usableDate(invoice.dueDate) ?? null;
