...
import { deriveUpcomingInvoice } from "./upcomingBill";
...
describe("deriveUpcomingInvoice", () => {
  ...
  test("has no due date when the provider names none, or an unusable one", () => {
    expect(derive({ dueDate: null }).dueAt).toBeNull();
    expect(derive({ dueDate: undefined }).dueAt).toBeNull();
    expect(derive({ dueDate: new Date("nope") }).dueAt).toBeNull();
  });
  ...
  test("reports a balance none of which this bill touches", () => {
    const bill = derive({
      customerBalanceApplied: 0,
      customerBalanceRemaining: 3200,
    });
    expect(bill.balanceApplied).toBeNull();
    expect(bill.balanceRemaining?.amountText).toBe("$32.00");
  });
