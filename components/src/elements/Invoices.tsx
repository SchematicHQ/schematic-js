...
import { deriveInvoiceList, type InvoiceRow } from "./model";
...
/** The wire type is a bare string cast, so a status the API adds arrives
 * with no entry here and `statusLabel` falls back to the raw value. */
...
/** The company's invoice history as a table of dated links. */
export function Invoices({
  ...
  // a card without a header would announce as a table of no stated subject.
      ...
      {list !== undefined && (
        <>
            ...
            <div className="schematic-header">
              ...
              {rows.length > 0 && (
                <span className="schematic-small schematic-muted schematic-invoices__count">
                  {/* The company's invoices, not the rows loaded: a card
                      showing twelve of eighty-four says so rather than
                      counting itself. Through the translator like every
                      other string here — the connective word and the plural
                      are the host's to own. */}
                  {visible.length < list.count
                    ? t("invoicesShowing", {
                        count: list.count,
                        shown: visible.length,
                      })
                    : t("invoicesCount", { count: list.count })}
                </span>
              )}
            </div>
          )}
          {rows.length === 0 ? (
            ...
            <table
              className="schematic-invoices__table"
