...
The keys this element renders are `invoicesHeader`, `invoicesCount` and
`invoicesShowing` (the count beside it, whole or partial), `invoicesLoading`,
`invoicesEmpty`, `invoicesDateColumn`, `invoicesAmountColumn`,
`invoicesStatusColumn`, `invoicesSeeMore`, `invoicesSeeLess`,
`invoicesLoadMore`, `invoicesCredit`, `invoicesUndated`, `retry`, and one per
status —
...
<div class="schematic-card schematic-invoices" data-state="ready">
  <div class="schematic-header">
    ...
    <!-- the company's invoices, omitted when there are none; reads
         "3 of 14 invoices" while rows are still unloaded -->
