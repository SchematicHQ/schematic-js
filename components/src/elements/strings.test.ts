...
const KEYS = [
  ...
  "paymentMethodsExpiresInMonths",
  "paymentMethodsFormError",
  "paymentMethodsFormLoading",
  ...
  "paymentMethodsRemoveError",
  "paymentMethodsSave",
  "paymentMethodsSaveError",
  "paymentMethodsSaving",
  "paymentMethodsSelectExisting",
...
describe("the string contract", () => {
  test("the key list is what hosts translate against", () => {
    expect([...bareKeys].sort()).toEqual(KEYS);
  });

  test("every key has a non-empty English default", () => {
    for (const key of bareKeys) {
      expect(DEFAULT_STRINGS[key as keyof typeof DEFAULT_STRINGS]).toBeTruthy();
    }
  });
