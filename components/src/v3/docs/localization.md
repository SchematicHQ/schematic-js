...
`STRING_KEYS` is the full key list and `ElementStrings` types it, so an
editor completes the keys and a rename fails the build rather than silently
falling back.
...
```tsx
import { DEFAULT_STRINGS } from "@schematichq/schematic-components/v3";
...
```ts
t("invoicesShowing", { count: rows.length });
```
...
`@schematichq/schematic-components/v3/locales/en.json` is the English copy in
...
function History() {
  ...
  const { rows } = deriveInvoiceList(page, { locale });
  return (
    <>
