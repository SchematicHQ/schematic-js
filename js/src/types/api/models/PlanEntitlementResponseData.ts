...
export interface PlanEntitlementResponseData {
  ...
  /**
   * How often overage charges are assessed and invoiced. Null or end_of_billing_period means the billing provider aggregates usage over the subscription's own period and bills it at period end. Only applies to overage price behavior.
   * @type {BillingArrearsCadence}
   * @memberof PlanEntitlementResponseData
   */
  overageBillingCadence?: BillingArrearsCadence | null;
  /**
   * Which boundary closes a monthly overage window: the subscription's own recurrence (billing_period_start) or the calendar month (month_end). Only meaningful when overage_billing_cadence is monthly.
