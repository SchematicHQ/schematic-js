 ...
 *
 * A naive `word + "s"` gets the common cases wrong for exactly the nouns
 * feature and credit names use ("entry" -> "entrys", "person" -> "persons"),
 * so the full rule set is carried here rather than approximated.
 */
...
function replace(word: string, rule: Rule): string {
  return word.replace(rule[0], (...args: unknown[]): string => {
    const match = args[0] as string;
    // For a rule without capture groups args[1] is the match offset, which is
    // what the zero-length-match branch below wants; with capture groups it is
    // the first group and that branch is unreachable.
    const index = args[1] as number;
    const result = rule[1].replace(/\$(\d{1,2})/g, (_, group: string) => {
      const value = args[Number(group)];
...
function sanitizeWord(
  ...
  if (token.length === 0 || token in uncountables) {
    return word;
  }
...
function pluralize(word: string, count?: number, inclusive?: boolean): string {
  ...
  return (
    (inclusive === true && count !== undefined ? `${count} ` : "") + pluralized
  );
}
