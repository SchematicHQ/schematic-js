...
describe("SchematicCompanyClient", () => {
  ...
  it("says a session started when the host first states one", () => {
    ...
    // Started, not changed: whatever is loaded was seeded for this session
    // by a host that prefetched before its auth resolved.
  ...
  it("treats a rebuilt token closure for the same key as the same session", async () => {
    ...
    expect(events).toEqual([]);
    // And the credential in hand still stands: it belongs to this session.
  ...
  it("still refreshes a 401 when the host restates the same session", async () => {
    // A host rebuilds `{ key, token }` every render, so the session object
    // is new while the company is not. Comparing objects rather than
    // counting changes would read that as a different session and hand the
    // reader the 401 instead of retrying it.
    let issued = 0;
    const mint = async () => `t${++issued}`;
    const { calls, fetchImpl } = fakeFetch((_url, headers) =>
      headers["X-Schematic-Api-Key"] === "t1"
  ...
  it("abandons the retry when the company changes while the token mints", async () => {
    ...
    // The refreshed token was minted, and then not sent: it is the new
    // company's to use.
  ...
  it("stops serving the token a 401 has already rejected", async () => {
    ...
    await client.fetchInvoices({ limit: 1, offset: 0 });
    await client.fetchInvoices({ limit: 1, offset: 0 });
    // t1 was rejected once; nothing sends it again.
  ...
  it("survives a logout landing mid-request", async () => {
    ...
    // The 401 surfaces as an API error; re-reading the ended session would
