 ...
 *
 * A keyed resource is seeded under the parameters in `params`, and under its
 * defaults when they are absent — so a prefetch for a non-default query is
 * read by the element asking that query rather than sitting unclaimed while
 * the element fetches the same rows again.
 */
export type CompanyData = {
  ...
  /**
   * The company these rows were fetched for. A prefetch outlives the render
   * that made it — a cached page, a session resolving to somebody else — so
   * a store adopts it only for the session it names, and refetches
   * otherwise. `fetchCompanyData` fills it in; a fixture need not.
   */
  sessionKey?: string;
};
