...
describe("goldens", () => {
  it("company_invoices decodes through the generated models", () => {
    ...
    // The envelope with it: the recording harness now carries `params`
    // through as raw JSON, so what is frozen here is the whole body the API
    // sends rather than the rows alone.
    requireContract(
      typeof body === "object" &&
        body !== null &&
        instanceOfGetCompanyInvoicesResponse(body),
      "company_invoices",
    );

    const response = GetCompanyInvoicesResponseFromJSON(body);
    requireContract(
      instanceOfCompanyInvoicesResponseData(response.data),
      "company_invoices data",
    );
